    ...
    <table border="1" width="100%"  align="center" cellspacing="0" cellpadding="0">
      ...
      <c:forEach var="kc" items="${requestScope.kc}">
          <tr align="center">           
              ...
              <td>${kc.dname}</td>
              <td>${kc.knumber}</td>
              <td><a href="Cuku1Servlet.do?did=${kc.did }">出库</a></td>    
          </tr>
      </c:forEach>
    </table> 
    <p>
      ...
      <c:if test="${nowPage>1 }">
      <a href="Kc2Servlet.do">首页</a>
      <a href="Kc2Servlet.do?page=${nowPage-1}">上一页</a>
      </c:if>
      <input type="text" value="${nowPage}" size=1 readonly="readonly"/>
      <c:if test="${nowPage<allPage}">
      <a href="Kc2Servlet.do?page=${nowPage+1 }">下一页</a>
      <a href="Kc2Servlet.do?page=${allPage}">尾页</a>
      </c:if>
      <c:if test="${nowPage==allPage}">
               下一页   尾页
      </c:if>
